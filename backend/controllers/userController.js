...
exports.registerUSer = async (req, res) => { //api to register new user
  try {
    ...
    res.status(400).json({ error: "could not register user : " + error.message });
  }
};
    ...
    if (passOk) {
      ...
      const callback = (err, token) => {
        if (err) {
          ...
          throw err;}
        res.cookie("token", token).json(payload); //putting token inside cookie
      };
...
exports.getProfile = async (req, res) => { //api to get profile of user
  ...
  try {
    ...
    jwt.verify(token, publicKey, { algorithms: "RS256" }, (err, info) => {
      ...
      if (err) {console.log("error in getting profile : ",err);
